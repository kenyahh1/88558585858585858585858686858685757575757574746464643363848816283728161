    ...
    local function LoadVxnityHub()
            ...
            Callback = function(Value)
                ...
                if Value then
                    ...
                    reachConnection = RunService.RenderStepped:Connect(function()
                        ...
                        -- GARANTIZAR CanCollide = false en la ball (sin alterar física)
                        pcall(function()
                            if tps.CanCollide then tps.CanCollide = false end
                        end)
                        ...
                        if distance <= reachDistance then
                            ...
                            if preferredFoot then
                                ...
                                if limb then
                                    -- FireTouchInterest SOLO — sin ninguna modificación de física
        ...
        local function startHeadReach()
            ...
            headConnection = RunService.RenderStepped:Connect(function()
                ...
                -- Asegurar CanCollide = false en la ball
                pcall(function()
                    if tps.CanCollide then tps.CanCollide = false end
                end)
        ...
        -- ============================================================
        -- REACTS — potentes, ball pegado sin delay
        -- ============================================================
        local currentReactPower = 0
        local reactHookEnabled = false

        -- Cache de RemoteEvent names para lookup O(1) via hash
        local REACT_NAMES = {
            Kick=true, KickC1=true, Tackle=true, Header=true,
            SaveRA=true, SaveLA=true, SaveRL=true, SaveLL=true, SaveT=true
        }

        -- Cache de ball para evitar búsqueda en cada frame
        local _cachedBall = nil
        local function getBallDirect()
            if _cachedBall and _cachedBall.Parent then return _cachedBall end
            local tps = Workspace:FindFirstChild("TPSSystem")
            _cachedBall = tps and tps:FindFirstChild("TPS")
            return _cachedBall
        end

        -- Invalidar cache si la ball cambia
        local _tpsSys = Workspace:FindFirstChild("TPSSystem")
        if _tpsSys then
            _tpsSys.ChildAdded:Connect(function(c) if c.Name=="TPS" then _cachedBall=c end end)
            _tpsSys.ChildRemoved:Connect(function(c) if c.Name=="TPS" then _cachedBall=nil end end)
        end
        Workspace.ChildAdded:Connect(function(c)
            if c.Name=="TPSSystem" then
                _tpsSys = c
                c.ChildAdded:Connect(function(ch) if ch.Name=="TPS" then _cachedBall=ch end end)
                c.ChildRemoved:Connect(function(ch) if ch.Name=="TPS" then _cachedBall=nil end end)
            end
        end)

        -- ═══════════════════════════════════════════════════
        -- applyReactInstant: 0 delay real, triple sincronización
        -- 1) SetNetworkOwner → propiedad del cliente
        -- 2) CanCollide=false
        -- 3) Velocidad en dirección + componente vertical suave
        -- 4) AssemblyLinearVelocity directo (sin intermediario)
        -- ═══════════════════════════════════════════════════
        local function applyReactInstant(power)
            ...
            -- Ownership primero para que los cambios sean autoritativos
            pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
            pcall(function() ball.CanCollide = false end)

            local lookDir = hrp.CFrame.LookVector
            -- Dirección combinada: look + ligero upward lift para sensación de control
            local reactDir = (lookDir + Vector3.new(0, 0.08, 0)).Unit
            ball.AssemblyLinearVelocity = reactDir * power
            ball.AssemblyAngularVelocity = Vector3.zero  -- eliminar spin no deseado
        end

        -- ═══════════════════════════════════════════════════
        -- enableReactHook: intercepción de FireServer con
        -- ejecución SINCRÓNICA (sin task.spawn) para 0ms delay
        -- ═══════════════════════════════════════════════════
        local function enableReactHook()
            ...
            meta.namecall = newcclosure(function(self, ...)
                ...
                -- Check rápido con hash table O(1)
                if method == "FireServer" and currentReactPower > 0 then
                    if REACT_NAMES[tostring(self)] then
                        local ball = getBallDirect()
                        local char = LocalPlayer.Character
                        local hrp = char and char:FindFirstChild("HumanoidRootPart")
                        if ball and hrp then
                            -- EJECUCIÓN SINCRÓNICA — mismo frame, 0 delay
                            pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                            pcall(function() ball.CanCollide = false end)
                            local lookDir = hrp.CFrame.LookVector
                            local reactDir = (lookDir + Vector3.new(0, 0.08, 0)).Unit
                            ball.AssemblyLinearVelocity = reactDir * currentReactPower
        ...
        ReactTab:Button({
            ...
            Desc = "W react",
            Callback = function()
                currentReactPower = 1e8
                enableReactHook()
                applyReactInstant(currentReactPower)
                VxnityUI:Notify({ Title = "React Active", Desc = "asolixun react enabled", Duration = 2 })
            end
        })
        ...
        ReactTab:Button({
            ...
            Desc = "goated react?",
            Callback = function()
                currentReactPower = 1e9
                enableReactHook()
                applyReactInstant(currentReactPower)
                VxnityUI:Notify({ Title = "React Active", Desc = "marianito react enabled", Duration = 2 })
            end
        })

        ReactTab:Button({
            Title = "✝️ - React Kenyah",
            Desc = "El mejor react — 0 ms",
            Callback = function()
                currentReactPower = 1e10
                enableReactHook()
                applyReactInstant(currentReactPower)
                VxnityUI:Notify({ Title = "React Kenyah", Desc = "React Kenyah enabled — máx potencia", Duration = 2 })
            end
        })

        ReactTab:Button({
            Title = "⚡ - No delay",
            Desc = "0 Delay — literalmente",
            Callback = function()
                currentReactPower = 5e9
                enableReactHook()
                applyReactInstant(currentReactPower)
                VxnityUI:Notify({ Title = "No Delay React", Desc = "0 delay activado", Duration = 2 })
            end
        })

        ReactTab:Button({
            Title = "⚡ - Ultra Velocity",
            Desc = "Velocidad máxima — ball instantáneo",
            Callback = function()
                currentReactPower = 2e10
                enableReactHook()
                applyReactInstant(currentReactPower)
                VxnityUI:Notify({ Title = "Ultra Velocity", Desc = "Velocidad extrema activada", Duration = 2 })
            end
        })
        ...
        ReactTab:Button({
            ...
            Callback = function()
                currentReactPower = 8e9
                enableReactHook()
                applyReactInstant(currentReactPower)
                VxnityUI:Notify({ Title = "Mizaku React", Desc = "Control absoluto activado", Duration = 2 })
            end
        })
        ...
        -- Slider de potencia custom
        ReactTab:Section({ Title = "Custom React Power" })
        ReactTab:Slider({
            Title = "React Power",
            Desc = "Ajuste manual de potencia",
            Value = { Min = 1e6, Max = 1e11, Default = 1e8 },
            Callback = function(val)
        ...
        -- ============================================================
        -- KENYAH INF HELPER — prioridad maxima de render, lerp inteligente
        -- ============================================================
        HelpersTab:Toggle({
            Title = "Kenyah Inf Helper",
            Desc = "aerial inf — potente y optimizado",
            Callback = function(state)
                if state then
                    local ball
                    local lastBallSearch = 0

                    -- BindToRenderStep First priority = ejecuta ANTES de animaciones y camara
                    RunService:BindToRenderStep("KenyahInfUltra", Enum.RenderPriority.First.Value, function()
                        local character = LocalPlayer.Character
                        if not character then return end
                        ...
                        -- Throttle: no buscar ball cada frame (cada 80ms)
                        local now = tick()
                        if not ball or not ball.Parent then
                            if now - lastBallSearch > 0.08 then
                                lastBallSearch = now
                                ball = nil
                                local tps = Workspace:FindFirstChild("TPSSystem")
                                if tps then ball = tps:FindFirstChild("TPS") end
                                if not ball then
                        ...
                        pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                        pcall(function() if ball.CanCollide then ball.CanCollide = false end end)

                        local base = torso or root
                        local targetPos = base.Position + base.CFrame.LookVector * 0.65 + Vector3.new(0, 0.55, 0)
                        local diff = targetPos - ball.Position
                        local dist = diff.Magnitude

                        if dist > 0.04 then
                            -- Velocidad proporcional: lejos=rapido, cerca=suave (sin jitter)
                            local speed = math.clamp(dist * 22, 25, 140)
                            ball.AssemblyLinearVelocity = diff.Unit * speed
                        else
                            -- Snap final sin vibracion
                            ball.CFrame = CFrame.new(targetPos)
                            ball.AssemblyLinearVelocity = Vector3.zero
                        end
                        ball.AssemblyAngularVelocity = Vector3.zero
                    end)
                    _G.AerialInfUltra = true
                else
                    if _G.AerialInfUltra then
                        RunService:UnbindFromRenderStep("KenyahInfUltra")
        ...
        -- ============================================================
        -- KENYAH INF TER/AIR — helper ultra potente con lerp adaptativo
        -- ============================================================
        HelpersTab:AddToggle({
            Title = "Kenyah INF TER/AIR [HELPER]",
            Desc = "Helper que guia el balon a velocidad brutal sin paralizarlo",
            Callback = function(state)
                if state then
                    local ball
                    local lastBallSearch = 0

                    -- RenderStepped con prioridad alta para ter/air
                    RunService:BindToRenderStep("KenyahINFTerAir", Enum.RenderPriority.First.Value + 1, function()
                        local char = LocalPlayer.Character
                        ...
                        -- Offset ajustado: 0.3 delante + 0.5 arriba = pies exactos
                        local magnetPos = base.Position + base.CFrame.LookVector * 0.30 + Vector3.new(0, 0.50, 0)
                        local direction = magnetPos - ball.Position
                        local dist = direction.Magnitude

                        if dist > 0.04 then
                            -- Fuerza adaptativa suave: escala con distancia, clampada para no explotar
                            -- dist < 1 = fuerza suave; dist > 3 = fuerza maxima
                            local forceMag = math.clamp(dist * 3500, 300, 75000)
                            ball.AssemblyLinearVelocity = direction.Unit * forceMag
                            ball.AssemblyAngularVelocity = Vector3.zero
                        else
                            -- Pegado: CFrame snap + zero velocity = sin vibracion
                    ...
                    if _G.KenyahINF then
                        RunService:UnbindFromRenderStep("KenyahINFTerAir")
                        _G.KenyahINF = nil
                    end
                end
            end
        ...
        RunService.RenderStepped:Connect(function()
            if followBall and toggleEnabled_follow then
        ...
        -- ============================================================
        -- INF HELPER AVANZADO (segundo, con CONFIG completo)
        -- ============================================================
        ...
        -- Helpers internos para el helper avanzado
        ...
        -- ============================================================
        -- LOOP PRINCIPAL DEL HELPER AVANZADO
        -- BindToRenderStep First priority = maxima precedencia
        -- Predicion mejorada con velocidad de HRP suavizada (no raw delta)
        -- ============================================================
        local lastHRPVel = Vector3.zero   -- velocidad suavizada del jugador
        local VEL_SMOOTH = 0.25           -- factor de suavizado (0=sin suavizado, 1=congelado)

        RunService:BindToRenderStep("VxnityInfHelperMain", Enum.RenderPriority.First.Value + 2, function(dt)
            ...
            -- Matar rotacion (doble: constraint + directo)
            ...
            if multiLockActive then
                ...
                if lockDist > CONFIG.LOCK_RADIUS then
                    local lockForce = toLock.Unit * CONFIG.MAGNET_PULL
                    lv.VectorVelocity = lockForce
                    ball.AssemblyLinearVelocity = lockForce
                else
                    lv.VectorVelocity = Vector3.zero
                    ball.AssemblyLinearVelocity = Vector3.zero
                end
            ...
            local targetPos
            if predictMode then
                -- Velocidad del HRP suavizada con lerp exponencial (sin jitter)
                local rawVel = hrp.AssemblyLinearVelocity
                lastHRPVel = lastHRPVel:Lerp(rawVel, 1 - VEL_SMOOTH)
                local predictedAhead = lastHRPVel * CONFIG.PREDICT_OFFSET * dt * 10
                targetPos = hrpPos
                    + hrp.CFrame.LookVector * CONFIG.FOLLOW_DISTANCE
                    + predictedAhead
            ...
            -- MAGNET MODE — pegado instantaneo con snap final
            if magnetMode then
                if toTargetDist > 0.035 then
                    -- Fuerza escalar: mas distancia = mas fuerza (suave pero responsiva)
                    local mag = math.clamp(toTargetDist * 3200, 400, CONFIG.MAGNET_PULL)
                    lv.VectorVelocity = toTarget.Unit * mag
                    ball.AssemblyLinearVelocity = toTarget.Unit * mag
                else
                    ball.CFrame = CFrame.new(targetPos)
                    lv.VectorVelocity = Vector3.zero
                    ball.AssemblyLinearVelocity = Vector3.zero
            ...
            -- Logica estandar con zonas de fuerza
            ...
            elseif dist > CONFIG.DEAD_ZONE then
                local dir = toTarget.Unit
                -- Speed adaptativa: escala suave con la distancia
        ...
        RunService.Heartbeat:Connect(function()
            ...
            -- CanCollide = false siempre en backup también
            pcall(function() if ball.CanCollide then ball.CanCollide = false end end)
