...
local function LoadVxnityHub()
    ...
    local function applyReactInstant(power)
        ...
        -- Garantizar CanCollide = false ANTES de aplicar la fuerza
        pcall(function() ball.CanCollide = false end)
        pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
        -- Aplicar velocidad en la dirección del jugador instantáneamente
        ...
        meta.namecall = newcclosure(function(self, ...)
            ...
            if tostring(method) == "FireServer" and currentReactPower > 0 then
                ...
                if table.find({"Kick", "KickC1", "Tackle", "Header", "SaveRA", "SaveLA", "SaveRL", "SaveLL", "SaveT"}, name) then
                    -- Aplicar react INMEDIATAMENTE (sin task.spawn para 0 delay real)
                    local ball = getBallDirect()
                    local char = LocalPlayer.Character
                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                    if ball and hrp then
                        ...
                        task.spawn(function()
                            ...
                            h.Adornee = ball
                            h.Color3 = Color3.fromRGB(220, 30, 30)
                            h.LineThickness = 0.15
                            task.delay(0.15, function() pcall(function() h:Destroy() end) end)
                        end)
                    end
                end
            end
            ...
            if state then
                ...
                _G.AerialInfUltra = RunService.Heartbeat:Connect(function()
                    ...
                    if not ball or not ball.Parent then
                        ...
                        -- Buscar también dentro de TPSSystem
                        if not ball then
                            local tps = Workspace:FindFirstChild("TPSSystem")
                    ...
                    -- CanCollide SIEMPRE false, sin alterar otras propiedades
                    ...
                    -- CFrame directo = 0 delay, pegado instantáneo
                    pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
        ...
        Callback = function(state)
            if state then
                ...
                _G.KenyahINF = RunService.RenderStepped:Connect(function()
                    ...
                    -- Cache de ball, re-buscar si se invalida
                    if not ball or not ball.Parent then
                        ball = nil
                        local tps = Workspace:FindFirstChild("TPSSystem")
                        if tps then ball = tps:FindFirstChild("TPS") end
                    ...
                    -- CanCollide = false SIEMPRE, sin loops extras
                    pcall(function() if ball.CanCollide then ball.CanCollide = false end end)
                    pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                    ...
                    if dist > 0.01 then
                        -- Fuerza proporcional a la distancia = más lejos → más fuerza (sin números ridículos)
                        local forceMag = math.clamp(dist * 2000, 200, 50000)
                        ball.AssemblyLinearVelocity = direction.Unit * forceMag
                    else
                        -- Ball ya pegado: suavizar
    ...
    local CONFIG = {
        ...
        STRONG_PULL      = 99999,
        SOFT_PULL        = 6000,
        MAGNET_PULL      = 150000,      -- fuerza modo imán (potente pero sin número ridículo)
        PREDICT_OFFSET   = 1.5,         -- studs adelante de predicción (razonable)
        VERTICAL_OFFSET  = -0.3,
        LOCK_RADIUS      = 0.05,
        ANGULAR_KILL     = true,
    }
    ...
    -- RenderStepped — lógica principal del helper avanzado
    RunService.RenderStepped:Connect(function()
        ...
        -- Garantizar CanCollide = false ANTES de cualquier física
        pcall(function() if ball.CanCollide then ball.CanCollide = false end end)
        pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
        ...
        -- MAGNET MODE — pegado instantáneo
        if magnetMode then
            if toTargetDist > 0.01 then
                ...
                -- Ya pegado: usar CFrame directo para 0 delay
