...
local function LoadVxnityHub()
    ...
    -- Potencia máxima segura (número finito máximo de Lua)
    ...
    local function applyReactInstant(power)
        ...
        ball.AssemblyLinearVelocity = hrp.CFrame.LookVector * power
    end
    ...
    local function enableReactHook()
        ...
        -- Pre-build lookup table para O(1) en lugar de table.find O(n) cada vez
        local reactActions = {
            Kick=true, KickC1=true, Tackle=true, Header=true,
            SaveRA=true, SaveLA=true, SaveRL=true, SaveLL=true, SaveT=true
        }

        -- Cache de ball y hrp para evitar lookups dentro del namecall crítico
        local _cachedBall, _cachedHRP = nil, nil
        local _ballCacheFrame = 0

        -- Actualizar cache en RenderStepped (fuera del namecall = sin overhead)
        local _cacheConn = RunService.RenderStepped:Connect(function()
            _ballCacheFrame = _ballCacheFrame + 1
            -- Re-cachear cada 2 frames para balance velocidad/costo
            if _ballCacheFrame >= 2 then
                _ballCacheFrame = 0
                local sys = Workspace:FindFirstChild("TPSSystem")
                _cachedBall = sys and sys:FindFirstChild("TPS")
                local char = LocalPlayer.Character
                _cachedHRP = char and char:FindFirstChild("HumanoidRootPart")
            end
        end)

        meta.namecall = newcclosure(function(self, ...)
            local method = getnamecallmethod()
            if method == "FireServer" and currentReactPower > 0 and reactActions[tostring(self)] then
                -- Ejecución INMEDIATA y SÍNCRONA en el hot path (0 delay real)
                local ball = _cachedBall
                local hrp  = _cachedHRP
                if ball and ball.Parent and hrp then
                    -- CanCollide guardian inline — sin pcall para max velocidad
                    if ball.CanCollide then
                        pcall(function() ball.CanCollide = false end)
                    end
                    pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                    -- Velocidad directa en la dirección del jugador
    ...
    -- ============================================================
    -- KENYAH INF HELPER — optimizado, potente, sin matar CanCollide mal
    -- ============================================================
    HelpersTab:Toggle({
        ...
        Callback = function(state)
            if state then
                local _ball = nil  -- cache persistente de ball
                ...
                _G.AerialInfUltra = RunService.RenderStepped:Connect(function()
                    ...
                    -- Cache de ball: solo re-buscar si se invalidó
                    if not _ball or not _ball.Parent then
                        local sys = Workspace:FindFirstChild("TPSSystem")
                        _ball = sys and sys:FindFirstChild("TPS")
                        if not _ball then
                    ...
                    -- CanCollide guardian sin pcall (ya está guardado por el guardian global)
                    if _ball.CanCollide then pcall(function() _ball.CanCollide = false end) end
                    pcall(function() _ball:SetNetworkOwner(LocalPlayer) end)

                    -- Offset CERO visual: la ball queda justo a los pies del torso
                    -- desde perspectiva externa = 0 reach visible
                    local targetPos = base.Position
                        + base.CFrame.LookVector * 0.4
                        + Vector3.new(0, 0.3, 0)

                    -- CFrame directo = snap instantáneo, 0 delay de interpolación
                    _ball.CFrame = CFrame.new(targetPos)
                    -- Velocidad de salida alineada al jugador (para que responda al kick)
                    _ball.AssemblyLinearVelocity = base.CFrame.LookVector * 30
    ...
    -- ============================================================
    -- KENYAH INF TER/AIR — helper ultra potente con fuerza aditiva
    -- ============================================================
    HelpersTab:AddToggle({
        Title = "Kenyah INF TER/AIR [HELPER]",
        Desc = "Helper ultra preciso — 0 reach visual, ball pegado",
        Callback = function(state)
            if state then
                local _ball = nil   -- cache persistente
                ...
                _G.KenyahINF = RunService.RenderStepped:Connect(function()
                    ...
                    -- Cache de ball: re-busca solo si se invalidó (eficiente)
                    if not _ball or not _ball.Parent then
                        local sys = Workspace:FindFirstChild("TPSSystem")
                        _ball = sys and sys:FindFirstChild("TPS")
                        if not _ball then
                    ...
                    -- Objetivo pegado al torso con mínimo offset visual
                    -- desde perspectiva externa parece que la ball está en los pies
                    local targetPos = base.Position
                        + base.CFrame.LookVector * 0.35
                        + Vector3.new(0, 0.2, 0)

                    local diff = targetPos - _ball.Position
                    local dist = diff.Magnitude

                    if dist > 0.05 then
                        -- Velocidad proporcional a la distancia: cuanto más lejos, más fuerza
                        -- Pero con techo sensato para no distorsionar físicas
                        local speed = math.clamp(dist * 800, 80, 4800)
                        _ball.AssemblyLinearVelocity = diff.Unit * speed
                    else
                        -- Ya pegado: snap CFrame para 0 jitter residual
                        _ball.CFrame = CFrame.new(targetPos)
                        _ball.AssemblyLinearVelocity = base.CFrame.LookVector * 20
                    end
                end)
            else
    ...
    -- ============================================================
    -- INF HELPER AVANZADO — ultra optimizado
    -- ============================================================
    ...
    local CONFIG = {
        -- Distancia de follow muy corta = ball visualmente pegada sin reach
        FOLLOW_DISTANCE = 0.4,
        FOLLOW_SPEED    = 600,
        -- Dead zone mínima = responde prácticamente al instante
        DEAD_ZONE       = 0.3,
        -- MAX_DISTANCE pequeño = el helper actúa casi siempre (control total)
        MAX_DISTANCE    = 1.2,
        STRONG_PULL     = 1200,
        SOFT_PULL       = 400,
        -- MAGNET_PULL calibrado: suficiente para pegarse sin distorsionar físicas
        MAGNET_PULL     = 2200,
        -- PREDICT pequeño = anticipa sin exagerar el offset
        PREDICT_OFFSET  = 0.12,
    ...
    -- Helpers internos con cache por instancia
    ...
    local function getPredictedTarget(hrp)
        ...
        if lastHRPPos and dt > 0 and dt < 0.1 then
            -- Clamp dt para evitar predicciones exageradas en lag spikes
    ...
    -- RenderStepped principal — prioridad alta, lógica limpia y rápida
    RunService.RenderStepped:Connect(function()
        ...
        -- CanCollide guardian sin overhead
        if ball.CanCollide then pcall(function() ball.CanCollide = false end) end
        pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
        ...
        -- Matar rotación (reduce jitter visual)
        ...
        -- MAGNET MODE — prioridad máxima
        if magnetMode then
            if toTargetDist > 0.05 then
                -- Velocidad proporcional + techo para suavidad visual
                local speed = math.clamp(toTargetDist * 900, 60, CONFIG.MAGNET_PULL)
        ...
        -- Lógica estándar con zonas calibradas
    ...
    -- Heartbeat backup — solo actúa si ball se escapa
