...
-- ============================================================
-- COLORES — PALETA MODERNA V5 (dark + neon red/coral)
-- ============================================================
local ACCENT      = Color3.fromRGB(235, 45, 45)        -- Rojo neon moderno
local ACCENT2     = Color3.fromRGB(170, 18, 18)        -- Rojo profundo
local BG_DARK     = Color3.fromRGB(6, 4, 4)            -- Fondo negro puro con tinte
local BG_FRAME    = Color3.fromRGB(14, 6, 6)           -- Frame oscuro saturado
local BG_ELEM     = Color3.fromRGB(20, 9, 9)           -- Elemento oscuro rojizo
local TEXT_WHITE  = Color3.fromRGB(245, 240, 240)
local TEXT_GRAY   = Color3.fromRGB(130, 95, 95)
local TEXT_MID    = Color3.fromRGB(200, 165, 165)
local OUTLINE     = Color3.fromRGB(55, 18, 18)
local ACCENT_GLOW = Color3.fromRGB(255, 75, 75)        -- Coral/neon hover
...
function VxnityUI:CreateWindow(opts)
    ...
    -- ============================================================
    -- ANIMACIÓN DE APERTURA V5 — más rápida, más limpia
    -- ============================================================
    task.spawn(function()
        tweenBack(MainFrame, 0.38, {
            Size = UDim2.fromOffset(newSize, newSize),
            BackgroundTransparency = 0
        }):Play()

        task.wait(0.12)
        tweenExpo(Topbar, 0.25, { BackgroundTransparency = 0 }):Play()
        tweenExpo(topFix, 0.25, { BackgroundTransparency = 0 }):Play()
        tweenCirc(topAccentLine, 0.45, { Size = UDim2.new(1, 0, 0, 1) }):Play()
        mainStroke.Color = OUTLINE

        task.wait(0.07)
        tweenSine(titleLbl, 0.22, { TextTransparency = 0 }):Play()
        task.wait(0.04)
        tweenSine(authorLbl, 0.22, { TextTransparency = 0 }):Play()
        ...
        task.wait(0.07)
        tweenExpo(TabPanel, 0.25, { BackgroundTransparency = 0 }):Play()
        tweenExpo(Separator, 0.25, { BackgroundTransparency = 0 }):Play()
            ...
            if child:IsA("Frame") or child:IsA("TextLabel") then
                task.delay(delay, function()
                    if child:IsA("Frame") then
                        tweenQuint(child, 0.16, { BackgroundTransparency = 0 }):Play()
                    else
                        TweenService:Create(child, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { TextTransparency = 0 }):Play()
                    end
                end)
                delay = delay + 0.028
            end
...
local function LoadVxnityHub()
    ...
    -- ============================================================
    -- REACTS V5 — HOOK GLOBAL, PERSISTE SIEMPRE, 0 DELAY
    -- ============================================================
    local REACT_ACTIONS = {
    ...
    ReactTab:Section({ Title = "⚡ Reacts V5 — Ultra Rápidos" })
    ...
    ReactTab:Button({ Title = "marianito react", Desc = "Goated — potencia elevada",
        Callback = function()
            currentReactPower = 7e6; P.reactPower = currentReactPower
            enableReactHook(); applyReactInstant(currentReactPower)
            VxnityUI:Notify({ Title = "marianito react", Desc = "Activado — potencia alta", Duration = 2 })
        end
    })
    ReactTab:Button({ Title = "⚡ No delay", Desc = "0 Delay literal — instantáneo",
        Callback = function()
            currentReactPower = 1.2e7; P.reactPower = currentReactPower
            enableReactHook(); applyReactInstant(currentReactPower)
            VxnityUI:Notify({ Title = "No Delay", Desc = "0 delay activado", Duration = 2 })
        end
    })
    ReactTab:Button({ Title = "⚡ Ultra Velocity", Desc = "Velocidad extrema — ball instantáneo",
        Callback = function()
            currentReactPower = 2.4e7; P.reactPower = currentReactPower
            enableReactHook(); applyReactInstant(currentReactPower)
    ...
    ReactTab:Button({ Title = "🌀 tmbuzz react v1", Desc = "Control magnético — ball pegada sin jitter",
        Callback = function()
            currentReactPower = 3.2e7; P.reactPower = currentReactPower
            enableReactHook()
            local ball = _G._VxRBall; local hrp = _G._VxRHRP
            if ball and hrp then
                pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                if ball.CanCollide then pcall(function() ball.CanCollide = false end) end
                ball.CFrame = CFrame.new(hrp.Position + hrp.CFrame.LookVector * 0.25 + Vector3.new(0, 0.1, 0))
                ball.AssemblyLinearVelocity = hrp.CFrame.LookVector * (currentReactPower * ballSpeedMult)
                ball.AssemblyAngularVelocity = Vector3.zero
            end
            VxnityUI:Notify({ Title = "tmbuzz react v1", Desc = "ACTIVADO — magnético", Duration = 2 })
        end
    })
    ...
    ReactTab:Button({ Title = "🔴 erubar react v1", Desc = "Pivot rápido — 0 reach visible + ultra pegado",
        Callback = function()
            currentReactPower = 3.8e7; P.reactPower = currentReactPower
            enableReactHook()
            local ball = _G._VxRBall; local hrp = _G._VxRHRP
            if ball and hrp then
                pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                if ball.CanCollide then pcall(function() ball.CanCollide = false end) end
                ball.CFrame = CFrame.new(hrp.Position + hrp.CFrame.LookVector * 0.2)
                ball.AssemblyLinearVelocity = hrp.CFrame.LookVector * (currentReactPower * ballSpeedMult)
                ball.AssemblyAngularVelocity = Vector3.zero
            end
    ...
    ReactTab:Button({ Title = "✝️ Kenyah react v5", Desc = "Control absoluto — predicción + snap + 0 reach",
        Callback = function()
            currentReactPower = 5.5e7; P.reactPower = currentReactPower
            enableReactHook()
            local ball = _G._VxRBall; local hrp = _G._VxRHRP
            if ball and hrp then
                pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                ...
                ball.CFrame = CFrame.new(hrp.Position + look * 0.18 + Vector3.new(0, 0.05, 0))
                ball.AssemblyLinearVelocity = look * (currentReactPower * ballSpeedMult)
                ball.AssemblyAngularVelocity = Vector3.zero
            end
            VxnityUI:Notify({ Title = "Kenyah v5", Desc = "MÁXIMA POTENCIA — predicción activa", Duration = 2 })
        end
    })

    -- xrea react I — control aéreo total
    ReactTab:Button({ Title = "💠 xrea react I", Desc = "Aéreo — control total incluso en el aire",
        Callback = function()
            currentReactPower = 4.8e7; P.reactPower = currentReactPower
            enableReactHook()
            local ball = _G._VxRBall; local hrp = _G._VxRHRP
            if ball and hrp then
                pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                ...
                ball.CFrame = CFrame.new(hrp.Position + look * 0.2 + Vector3.new(0, 0.12, 0))
                ball.AssemblyLinearVelocity = dir * (currentReactPower * ballSpeedMult)
                ball.AssemblyAngularVelocity = Vector3.zero
            end
            VxnityUI:Notify({ Title = "xrea react I", Desc = "ACTIVADO — aéreo preciso", Duration = 2 })
        end
    })

    -- xrea react II — doble pulso
    ReactTab:Button({ Title = "⚡ xrea react II", Desc = "Doble pulso — respuesta instantánea sin rival",
        Callback = function()
            currentReactPower = 6.2e7; P.reactPower = currentReactPower
            enableReactHook()
            local ball = _G._VxRBall; local hrp = _G._VxRHRP
            if ball and hrp then
                pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                if ball.CanCollide then pcall(function() ball.CanCollide = false end) end
                local look = hrp.CFrame.LookVector
                ball.CFrame = CFrame.new(hrp.Position + look * 0.15)
                ball.AssemblyLinearVelocity = look * (currentReactPower * ballSpeedMult)
                ball.AssemblyAngularVelocity = Vector3.zero
                task.defer(function()
            ...
            VxnityUI:Notify({ Title = "xrea react II", Desc = "DOBLE PULSO — sin rival", Duration = 2 })
        end
    })

    -- xrea react III — omni con corrección automática
    ReactTab:Button({ Title = "🌐 xrea react III", Desc = "Omnidireccional — corrección de trayectoria auto",
        Callback = function()
            currentReactPower = 7.0e7; P.reactPower = currentReactPower
            enableReactHook()
            local ball = _G._VxRBall; local hrp = _G._VxRHRP
            if ball and hrp then
                pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                ...
                task.defer(function()
                    if ball and ball.Parent and hrp and hrp.Parent then
                        ...
                        local correction = (hrp.Position + newLook * 0.15 - ball.Position)
            ...
            VxnityUI:Notify({ Title = "xrea react III", Desc = "OMNI — auto-corrección activa", Duration = 2 })
        end
    })
    ...
    ReactTab:Slider({
        ...
        Value = { Min = 1e5, Max = 8e7, Default = 1e7 },
        Callback = function(val)
    ...
    -- KENYAH INF HELPER V5 — lerp dinámico, persiste
    HelpersTab:Toggle({ Title = "Kenyah Inf Helper", Desc = "aerial inf — pegado al pie, 0 reach visual",
        Callback = function(state)
            if state then
                _G.AerialInfUltra = RunService.RenderStepped:Connect(function()
                    ...
                    if ball.CanCollide then pcall(function() ball.CanCollide = false end) end
                    pcall(function() ball:SetNetworkOwner(LocalPlayer) end)
                    local targetPos = base.Position + base.CFrame.LookVector * 0.3 + Vector3.new(0, 0.1, 0)
                    local diff = targetPos - ball.Position
                    local dist = diff.Magnitude
                    if dist > 0.05 then
                        local spd = math.clamp(dist * 1600, 80, 9000)
                        ball.AssemblyLinearVelocity = diff.Unit * spd
                    else
                        ball.CFrame = CFrame.new(targetPos)
                        ball.AssemblyLinearVelocity = base.CFrame.LookVector * 30
                    end
                    ball.AssemblyAngularVelocity = Vector3.zero
                end)
            else
    ...
    -- KENYAH INF TER/AIR V5 — smoothing dinámico, 0 jitter, persiste
    HelpersTab:AddToggle({ Title = "Kenyah INF TER/AIR [HELPER]", Desc = "0 reach visual — ball pegada, persiste tras morir",
        Callback = function(state)
            if state then
                _G.KenyahINF = RunService.RenderStepped:Connect(function()
                    ...
                    local dist = diff.Magnitude
                    if dist > 0.04 then
                        local speed = math.clamp(dist * 1500, 120, 8500)
                        ball.AssemblyLinearVelocity = diff.Unit * speed
                    else
                        ball.CFrame = CFrame.new(targetPos)
                        ball.AssemblyLinearVelocity = base.CFrame.LookVector * 25
                    end
                    ball.AssemblyAngularVelocity = Vector3.zero
                end)
            else
    ...
    HelpersTab:Toggle({ Title = "inf helper", Desc = "Character will move towards the ball automatically",
        Callback = function(state)
